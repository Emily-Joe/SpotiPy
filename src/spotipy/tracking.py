...
def get_time_steps(file_array, directory):
    """
    calculates the time beween two consecutive FITS-files (in the order of thier position in file_name_array).
    To be corrosponsive to file_name_array, the first entry is set to 0, because the duration to the previous
    file is zero, since its the first.
    ...
    -------
    durations
        array of the amount of time between the corresponding two images in the name-list.
        This array corresponds to the file_name_array
...
def calculate_tracks(file_array, directory, x0, y0, time_steps):
    """
    Takes a starting position (in arcsec) and calculates the next position of this point after a desired
    time step in accordance with differential rotation rates at the given latitude of the starting point.
...
def track_region(image, header, tracks, frame_size, i):
    ...
    # check if frame is outside of original FITS files range.
    # This can happen with a big frame size and features close to either side of the solar edge
    # FAIL-SAVE:
    half_frame = int(frame_size//2)  # rename for convenience
    ...
    # (this last step was neccessary, because otherwise the nans would appear dark on png images
    # made from the data, which would mess with the center-finding process)

    # update frame locations in case fail-save was used:
    x_arc_f = (cx - x_f) * dx  # correct: pixel offset x scale
...
def strip(series, directory, tracks, strip_height_arcsec=None, frame_size=None, animate=False, overlay=True, save_path=None):
    ...
    strip_height_arcsec : float or None
        Total strip height in arcsec (default: 400)
    ...
    overlay : bool
        If True, the strip will consist of the summed images of all files in series
    ...
    # set up extent in (Solar Disc-) arcsec coordinates:
    x_left  = (0 - cx) * dx
    x_right = (W - cx) * dx
    ...
    if overlay:
        ...
        if not animate:
            ...
            ax.imshow(strip_img, origin='lower', cmap='hot',
            ...
            if save_path is not None:
                ...
                if yn == 'y':
                    plt.savefig(os.path.join(save_path, 'time_summed_strip.png'), dpi=300)
                    print("Saved.")
        ...
        for idx in range(len(tracks)):
            ...
            rect = plt.Rectangle(
                (x_arc - half_x, y_arc - half_y),
                2 * half_x, 2 * half_y,
                linewidth=2.5, edgecolor='dodgerblue', facecolor='none', alpha=0.95
            )
            ax.add_patch(rect)
            fig.tight_layout()
            canvas.draw()
            w, h = canvas.get_width_height()
            img_rgb = np.frombuffer(canvas.buffer_rgba(), dtype='uint8').reshape(h, w, 4)[:, :, :3]
    ...
    # ============================================================
        ...
        for i, fname in enumerate(series):
            ...
            fig, ax = plt.subplots(figsize=(12, 3))
            canvas = FigureCanvas(fig)
            ax.imshow(strip_img, origin='lower', cmap='gray',
                      extent=[x_left, x_right, y_min_arc, y_max_arc])
            ...
            if save_path is not None:
                frame_dir = os.path.join(save_path, 'strip_frames')
                os.makedirs(frame_dir, exist_ok=True)
                plt.imsave(os.path.join(frame_dir, f"frame_{i:04d}.png"), img_rgb)
...
def refine_centering(image_crop, current_x0, current_y0, frame_size):
    ...
    This prevents the spot from drifting out of the cropped window by calculating
